...
program
  ...
  .version('1.0.2\n' + pc.dim('Author : Matéo Florian Callec <mateo@callec.net>'), '-V, --version');
...
program
  ...
  .action((bytes: string, opts: { webcam: boolean; output?: string }) =>
    entropyCommand(bytes, opts).catch(e => {
      console.error(e);
      process.exit(1);
    }),
  );

...
function captureFrames(deviceIndex: number): Uint8Array[] {
  ...
  for (let i = 0; i < FRAME_COUNT; i++) {
    ...
    if (existsSync(out)) {
      try {
        frames.push(new Uint8Array(readFileSync(out)));
      } finally {
        unlinkSync(out);
      }
    }
...
export function harvestWebcamEntropy(deviceIndex = 0): Uint8Array {
  ...
  if (!frames.length)
    throw new Error('No webcam frames captured — is ffmpeg installed and a camera connected?');

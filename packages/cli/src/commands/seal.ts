...
export async function sealCommand(inputPath: string, opts: SealOptions): Promise<void> {
  ...
  try {
    ...
    if (mode === 'hybrid') {
      ...
      paraBuffer = encodePara(
        MODE_HYBRID,
        buildHeader(absInput, plaintext.length, nonce, mlkemCt, p521EphPk),
        ct,
      );
    } else if (mode === 'pqc') {
      ...
      paraBuffer = encodePara(
        MODE_PQC,
        buildHeader(absInput, plaintext.length, nonce, mlkemCt),
        ct,
      );
    } else {
      ...
      paraBuffer = encodePara(
        MODE_P521,
        buildHeader(absInput, plaintext.length, nonce, undefined, ephemeralPublicKey),
        ct,
      );
    }

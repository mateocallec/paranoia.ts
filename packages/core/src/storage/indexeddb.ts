 ...
 */
export async function loadKeyPair(wrappingKey: Uint8Array, id = 'default'): Promise<HybridKeyPair> {
  const db = await openDB();
...
export class SessionKeyStore {
  ...
  async set(id: string, sessionKey: Uint8Array): Promise<void> {
    ...
    blob.set(nonce);
    blob.set(ct, 12);
    sessionStorage.setItem(`paranoia:${id}`, btoa(String.fromCharCode(...blob)));
  }
...
function serializeKeyPair(kp: HybridKeyPair): Uint8Array {
  ...
  let off = 0;
  buf.set(kp.publicKey.mlkem, off);
  off += SIZES.MLKEM_PK;
  buf.set(kp.privateKey.mlkem, off);
  off += SIZES.MLKEM_SK;
  buf.set(kp.publicKey.p521, off);
  off += SIZES.P521_PK;

...
export function encodePara(
  ...
  return Buffer.concat([MAGIC, Buffer.from([VERSION, mode]), hdrLen, hdrJson, Buffer.from(ciphertext)]);
}

export function defaultParaPath(input: string): string { return `${input}.para`; }
...
export function decodePara(data: Buffer): ParsedPara {
  if (!data.subarray(0, 4).equals(MAGIC))
    throw new Error('Not a .para file (invalid magic)');
  if (data[4] !== VERSION)
...
export function buildHeader(
  file: string, size: number, nonce: Uint8Array,
  mlkemCt?: Uint8Array, p521EphPk?: Uint8Array,
): ParaHeader {
  return {
